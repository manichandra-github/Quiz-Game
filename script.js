const start_buttn = document.querySelector(".start_buttn button");
    ...
    if(userAns == correcAns){ 
        ...
        for(i=0; i < allOptions; i++){
            if(option_list.children[i].textContent == correcAns){  
                ...
                console.log("correct Answer is other Option.");
            }
        }
    }
    for(i=0; i < allOptions; i++){
...
function showResult(){
    ...
    if (userScore > 3){ 
        ...
        let scoreTag = '<span>BetterLuckNextTime , You got <p>'+ userScore +'</p> out of <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
    else{ 
        let scoreTag = '<span>BetterLuckNextTime, You got only <p>'+ userScore +'</p> out of <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
}
        ...
        if(time < 0){ 
            ...
            for(i=0; i < allOptions; i++){
                if(option_list.children[i].textContent == correcAns){ 
                    option_list.children[i].setAttribute("class", "option correct"); 
                    option_list.children[i].insertAdjacentHTML("beforeend", tickIconTag); 
                    console.log("Time Off: Auto select correct .");
                }
            }
            for(i=0; i < allOptions; i++){
                option_list.children[i].classList.add("disabled"); 
